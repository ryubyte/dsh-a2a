...
import type { JsonRpcRequest, JsonRpcResponse, JsonRpcSuccess, StreamResponse } from './protocol.js';
...
export class JsonRpcClient {
  ...
  async call<T>(method: string, params: unknown, options: JsonRpcCallOptions = {}): Promise<T> {
    ...
    try {
      ...
      if (!res.ok) {
        throw new A2AError(
          res.status,
          `A2A JSON-RPC request failed (HTTP ${res.status}) at ${this.url}`,
          undefined,
          true,
        );
      }
      ...
      return (json as JsonRpcSuccess).result as T;
    } catch (err) {
  ...
  async *stream<T = StreamResponse>(
    ...
    if (!res.ok || !res.body) {
      ...
      throw new A2AError(
        res.status,
        `A2A stream request failed (HTTP ${res.status}) at ${this.url}`,
        undefined,
        true,
      );
    }
