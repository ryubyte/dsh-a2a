...
export async function fetchAgentCard(
  ...
  const isExactCard =
    baseUrl.endsWith('agent-card.json') || baseUrl.endsWith('agent-card.json/');
  ...
  for (const url of candidates) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new A2AError(
          res.status,
          `AgentCard request failed (${res.status}) at ${url}`,
          undefined,
          true,
        );
      }
      ...
      if (!isAgentCard(json)) {
        throw new A2AError(
          400,
          `Invalid AgentCard at ${url}: missing required fields`,
          undefined,
          true,
        );
      }
...
export function pickInterface(
  card: AgentCard,
  preferred?: string,
): { iface: AgentInterface; url: string } {
  const ifaces = card.supportedInterfaces ?? [];
  if (ifaces.length === 0) {
    throw new A2AError(
      -32000,
      `AgentCard for "${card.name}" declares no supportedInterfaces`,
    );
  }
