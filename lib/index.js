...
var A2AServer = class {
	...
	*/
	setAuthToken(token) {
		this.options.authToken = token && token.length > 0 ? token : void 0;
		if (this.options.authToken) applyAuthScheme(this.card, this.options.authToken);
		else {
	...
	*/
	updateCard(patch) {
		...
		if (patch.endpointPath !== void 0 && patch.endpointPath !== this.endpointPath) {
			this.endpointPath = patch.endpointPath || "/a2a";
			endpointChanged = true;
		}
		if (patch.baseUrl !== void 0) this.baseUrl = patch.baseUrl.replace(/\/$/, "");
		if (patch.baseUrl !== void 0 || endpointChanged) this.rebuildInterfaceUrls();
	...
	async runTask(taskId, contextId, msg) {
		...
		try {
			...
			if (controller.signal.aborted || isTerminal(this.store.get(taskId)?.status.state)) return;
...
*/
function applyAuthScheme(card, token) {
	if (!token) return;
...
function partsToPrompt(parts) {
	return parts.map((p) => "text" in p && typeof p.text === "string" ? p.text : "").filter(Boolean).join("\n").trim();
}
...
function lastAssistantText(agent) {
	...
	for (let i = history.length - 1; i >= 0; i--) if (history[i].role === "assistant") return history[i].content.map((b) => b.type === "text" && typeof b.text === "string" ? b.text : "").filter(Boolean).join("\n").trim();
	return "";
}
/** Derive a stable session id for an A2A context id. */
function sessionIdForContext(contextId) {
...
*/
function createDshAgentExecutor(agents, opts) {
	...
	*/
	const agentFor = async (contextId) => {
		...
		if (presets) try {
			...
			throw new Error(`a2a: failed to resolve agent preset: ${err.message}`);
		}
		const createOptions = {
			sessionId,
			...presetId ? { meta: {
		...
		try {
			...
			if (/already (has|owns)|persisted/i.test(err.message)) handle = await agents.resume({
	...
	const runTurn = async (contextId, prompt, signal) => {
	...
	const executor = (async ({ message, contextId, signal }) => {
		...
		const run = (tails.get(ctxKey) ?? Promise.resolve()).catch(() => {}).then(() => runTurn(ctxKey, prompt, signal));
		...
		try {
			...
			return {
				messageId: crypto.randomUUID(),
				role: "ROLE_AGENT",
				parts: [{ text: `DSH agent executor error: ${err.message}` }]
			};
		} finally {
			if (tails.get(ctxKey) === run) tails.delete(ctxKey);
		}
	});
	executor.disposeAll = async () => {
	...
	async control(action, target, payload) {
		switch (action) {
			...
			case "discover-agent": {
				...
				const bearerToken = payload?.bearerToken || void 0;
				return this.hooks.discoverAgent ? this.hooks.discoverAgent(url, { bearerToken }) : {
					ok: false,
			...
			case "add-agent": {
				...
				const bearerToken = payload?.bearerToken || void 0;
				return this.hooks.addAgent ? this.hooks.addAgent(name, url, { bearerToken }) : {
					ok: false,
			...
			case "set-server-auth": {
				const token = payload?.authToken || void 0;
				return this.hooks.setServerAuthToken ? this.hooks.setServerAuthToken(token) : {
					ok: false,
					message: "set-server-auth is not wired (server mode not mounted)"
				};
			}
			...
			case "set-server-identity":
				if (!this.hooks.setServerIdentity) return {
					ok: false,
					message: "set-server-identity is not wired (server not mounted)"
				};
			...
			case "update-agent":
				if (!this.hooks.updateAgent) return {
					ok: false,
					message: "update-agent is not wired (client mode not mounted)"
				};
				return this.hooks.updateAgent(target, {
					timeoutMs: typeof payload?.timeoutMs === "number" ? payload.timeoutMs : void 0,
					mapSkills: typeof payload?.mapSkills === "boolean" ? payload.mapSkills : void 0,
					bearerToken: payload?.bearerToken || void 0
				});
			default: return {
				ok: false,
...
*/
function profileNameFromDshAlias() {
	const argv = process.argv ?? [];
	if (argv.includes("--profile") || argv.some((a) => a.startsWith("--profile="))) return void 0;
...
function resolveConfigPath() {
	...
	if (profileName && profileName !== "." && profileName !== ".." && !profileName.includes("/") && !profileName.includes("\\")) return join(home, "profiles", profileName, A2A_CONFIG_FILENAME);
	return cwdFile;
}
		...
		ctx.inject(["tools"], (ctx) => {
			...
			dashboard.setHooks({
				...
				updateAgent: async (connectionId, patch) => {
					...
					entry?.configured ?? snap?.configured;
					if (!name || !url) return {
						ok: false,
						message: `connection ${connectionId} not found`
					};
					...
					if (!found) return {
						ok: false,
						message: "only saved (configured) agents can be edited"
					};
					if (patch.timeoutMs !== void 0) found.item.timeoutMs = patch.timeoutMs;
					if (patch.mapSkills !== void 0) found.item.mapSkills = patch.mapSkills;
					if (patch.bearerToken !== void 0) found.item.bearerToken = patch.bearerToken || void 0;
					persistAgents();
					const item = found.item;
					if (item.enabled === false) return {
						ok: true,
						message: `agent ${connectionId} updated (config kept, still disabled)`
					};
					if (entry) {
						entry.disposeAll();
						live.delete(connectionId);
					...
					if (!newId) return {
						ok: false,
						message: `update of ${connectionId} failed to reconnect`
					};
					dashboard.removeAgent(connectionId);
					return {
						ok: true,
						message: `agent ${connectionId} updated as ${newId} (saved to ${configPath})`
					};
				}
			});
			ctx.effect(() => {
	...
	}
	{
		...
		ctx.inject(["webServer"], (ctx) => {
			ctx.effect(() => {
				...
				if (mergedConfig.baseUrl) try {
					const pinnedPort = new URL(mergedConfig.baseUrl).port;
					if (pinnedPort && Number(pinnedPort) !== webServer.port) logger?.warn?.(`[a2a] pinned baseUrl ${mergedConfig.baseUrl} port ${pinnedPort} != listen port ${webServer.port}; remote callers may be unreachable. Clear baseUrl in settings to auto-infer.`);
				} catch {}
				let execute = mergedConfig.execute;
				let executorKind = execute ? "custom" : "none";
				let disposeExecutor;
				if (!execute) {
					...
					if (agents) {
						...
						const dshExec = createDshAgentExecutor(agents, {
							...
							resolveAgentOptions: () => {
								...
								if (!sel?.model && !sel?.provider) return void 0;
								return {
				...
				} else logger?.info?.("[a2a] A2A server ready but not serving (enable via settings)");
				const serverCard = () => ({
				...
				dashboard.setHooks({
					setServerEnabled: async (enable) => {
						...
						if (!res.ok) {
							...
							return {
								ok: false,
								message: `applied in-memory but failed to save: ${res.message}`
							};
						}
						return {
							ok: true,
							message: enable ? "A2A server enabled" : "A2A server disabled"
						};
					},
					setServerIdentity: async (patch) => {
						const clearBaseUrl = patch.baseUrl === "";
						const cardPatch = clearBaseUrl ? {
							...patch,
							baseUrl: void 0
						} : patch;
						const { endpointChanged } = server.updateCard(cardPatch);
						if (clearBaseUrl) server.setBaseUrl(inferBaseUrl(webServer.host, webServer.port));
						...
						const next = loadConfig();
						const s = { ...next.server ?? {} };
						if (patch.agentName !== void 0) s.agentName = patch.agentName;
						if (patch.agentDescription !== void 0) s.agentDescription = patch.agentDescription;
						if (patch.agentVersion !== void 0) s.agentVersion = patch.agentVersion;
						if (clearBaseUrl) delete s.baseUrl;
						else if (patch.baseUrl !== void 0) s.baseUrl = patch.baseUrl;
						if (patch.endpointPath !== void 0) s.endpointPath = patch.endpointPath;
						if (patch.skills !== void 0) s.skills = patch.skills;
						next.server = s;
						const res = saveConfig(next);
						if (!res.ok) {
						...
						return {
							ok: true,
							message: `已更新对外服务 (saved to ${res.path})`
						};
					},
					setServerAuthToken: async (token) => {
						...
						next.server = {
							...next.server ?? {},
							authToken: token && token.length > 0 ? token : void 0
						};
						const res = saveConfig(next);
						if (!res.ok) {
						...
						return {
							ok: true,
							message: token ? "已设置入站鉴权 token" : "已清除入站鉴权 token"
						};
					},
					serverStatus: () => ({
						ok: true,
				...
				return () => {
					unregisterRoutes();
					disposeExecutor?.().catch((err) => logger?.error?.(`[a2a] executor dispose failed: ${err.message}`));
				};
			}, "a2a: server routes");
		});
	}
...
* `create()` rejects.
*/
function probeAgentRegistry(ctx) {
	return probeService(ctx, "agents", "create");
}
/**
...
*/
function probeAgentPresets(ctx) {
	return probeService(ctx, "agentPresets", "resolve");
}
/**
...
* model another way.
*/
function probeAgentDefaultModel(ctx) {
	return probeService(ctx, "agentDefaultModel", "currentSelection");
}
/** Probe `ctx.sessionTitle` (optional). Used to name inbound sessions. */
function probeSessionTitle(ctx) {
	return probeService(ctx, "sessionTitle", "rename");
}
/**
...
function probeService(ctx, name, method) {
	try {
		...
		if (svc && typeof svc[method] === "function") return svc;
	} catch {}
}
/**
...
*/
function summarize(prompt, maxChars = 40) {
	const text = prompt.replace(/\s+/g, " ").trim();
	if (text.length <= maxChars) return text || "(empty)";
	const cut = text.slice(0, maxChars);
	const sp = cut.lastIndexOf(" ");
	return `${(sp > maxChars * .6 ? cut.slice(0, sp) : cut).trimEnd()}…`;
}
/**
...
*/
function inboundSessionCwd(configPath) {
	try {
...
*/
function inferBaseUrl(host, port) {
	return `http://${host === "0.0.0.0" ? primaryLanAddress() ?? "127.0.0.1" : host || "127.0.0.1"}:${port}`;
}
/**
...
//#endregion
export { A2AClient, A2AError, A2AServer, A2A_ERROR_CODES, A2A_METHODS, DashboardRegistry, Role, TaskState, TaskStore, apply, createDshAgentExecutor, defaultExecutor, defaultSkills, fetchAgentCard, getSharedRegistry, inferBaseUrl, inject, isTerminal, name, notConfiguredExecutor, registerAgentTools, shellExecutor };
